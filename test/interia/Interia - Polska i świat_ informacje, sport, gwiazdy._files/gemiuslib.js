...
if (typeof gemius_hcconn == "undefined") {
	gemius_hcconn = {
		...
		hc : (typeof gemius_hitcollector != 'undefined')?gemius_hitcollector:(typeof pp_gemius_hitcollector != 'undefined')?pp_gemius_hitcollector:'stream-interia.hit.gemius.pl',
		...
		sonar : function(sid) {
			...
			while (prob>0) {
				if (data["sdur"]<24*3600 && prob<=4 && data["lvstate"]=="visible" && Math.random() < prob/data["freq"]) {
					gemius_hcconn.internal_hit(0,99,data["id"],data["evid"],"smpsonar",0,0,"_ASF="+data["freq"]);
				}
